...
function buildInsights(animals){
  ...
  animals.forEach(item=>{
    const a=item.a;
    const lastFeed=latest(a.feeds);
    const lastWeight=latest(a.weights);
    const feedDays=daysSince(lastFeed&&lastFeed.date);
    const weightDays=daysSince(lastWeight&&lastWeight.date);
    if(!lastFeed||feedDays>=14) feeding.push({d:lastFeed?lastFeed.date:'-',txt:a.name+': Fütterung prüfen'});
    if(!lastWeight||weightDays>=42) weights.push({d:lastWeight?lastWeight.date:'-',txt:a.name+': Gewicht aktualisieren'});
    const recent=(a.feeds||[]).slice().sort((x,y)=>String(y.date||'').localeCompare(String(x.date||''))).slice(0,3);
    if(recent.length>=3&&recent.every(f=>f.accepted===false)) refusals.push({d:recent[0].date,txt:a.name+': 3 Verweigerungen in Folge'});
  });
  (NGTStore.data().foodInventory||[]).forEach(f=>{
    if(Number(f.qty||0)<=3) food.push({d:'',txt:f.name+': niedriger Bestand ('+Number(f.qty||0)+')'});
  });
...
function render(){
  ...
  let recent=[];
  all.forEach(x=>{
    recent.push(...NGTUI.timeline(x.a).map(r=>({d:r.d,txt:x.a.name+': '+r.txt})));
  });
  recent.sort((a,b)=>String(b.d).localeCompare(String(a.d)));
  const ins=buildInsights(all);
  const notes=[...ins.feeding.slice(0,5),...ins.weights.slice(0,5),...ins.refusals,...ins.food];
  ...
  <div class="card"><h2>🤖 KI Assistent</h2><p class="muted">Schnelle Eingabe für Routineaufgaben.</p><button onclick="NGT500.route('assistant')">KI Assistent öffnen</button></div>
...
function search(q){
  ...
  const rows=NGTStore.allAnimals().filter(x=>JSON.stringify(x.a).toLowerCase().includes(q));
  document.getElementById('searchBox').innerHTML=q?rows.map(NGTUI.animalCard).join('')||'<p class="muted">Keine Treffer.</p>':'';
}
window.NGTDashboard={search};
NGT500.register('dashboard',{render});
})();
