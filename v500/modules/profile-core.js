...
P.opt=function(list,currentValue){
 return (list||[]).map(function(value){
  ...
  return '<option value="'+P.esc(value)+'" '+(selected?'selected':'')+'>'+P.esc(value)+'</option>';
 }).join('');
};
...
P.sexCode=function(value){
 ...
 if(value.includes('weib'))return '0.1';
 if(value.includes('männ')||value.includes('maenn'))return '1.0';
 return '0.0';
};

P.scientificName=function(animal){
 return AnimalEngine.getScientificName(animal)||'-';
};

P.action=function(icon,label,onclick){
 const action='<button class="tc2ProfileAction" onclick="'+onclick+'"><div class="tc2ProfileActionIcon">'+icon+'</div><div class="tc2ProfileActionText">'+P.esc(label)+'</div><div class="tc2ProfileActionArrow">›</div></button>';
 if(label!=='Gesundheit')return action;
 const context=P.state.ctx||{};
 const removeAction="NGTAnimals.remove('"+P.jsArg(context.t)+"',"+Number(context.i||0)+")";
 return action+'<button class="tc2ProfileAction danger" style="grid-column:1/-1;width:100%;min-height:68px;background:linear-gradient(180deg,#d63a3a,#b6222b)!important;border:1px solid rgba(255,125,125,.55)!important;color:#fff!important;box-shadow:0 14px 30px rgba(125,0,12,.28)!important" onclick="'+removeAction+'"><div class="tc2ProfileActionIcon">🗑️</div><div class="tc2ProfileActionText">Tier löschen</div><div class="tc2ProfileActionArrow" style="color:#fff!important">›</div></button>';
};

P.row=function(date,label,deleteAction){
 return '<div class="tc2ListRowFull"><div><b>'+P.esc(date||'-')+'</b><small>'+P.esc(label||'')+'</small></div><button class="danger" onclick="'+deleteAction+'">Löschen</button></div>';
};

P.setContext=function(args){
 const next=args||P.state.ctx||{};
 const current=P.state.ctx||{};
 const changed=String(next.t||'')!==String(current.t||'')||Number(next.i||0)!==Number(current.i||0);
 P.state.ctx=next;
 if(args&&args.tab)P.state.tab=args.tab;
 else if(changed)P.state.tab='overview';
};
