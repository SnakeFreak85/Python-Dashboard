...
function patchStorage(){if(window.__tcSyncPatched)return;window.__tcSyncPatched=true;const rawSet=localStorage.setItem.bind(localStorage);localStorage.setItem=function(k,v){rawSet(k,v);if(String(k)===STORE_KEY||String(k).startsWith('ngt_')||String(k).startsWith('terracontrol_settings'))setTimeout(()=>markDirty('Lokale Daten geändert'),0)}}
