...
function translateText(value,language){
 ...
 if(translated===trimmed&&currentLanguage==='en'){
  translated=translated
   ...
   .replace(/^(\d+) ausgewählt$/,function(_,count){return count+' selected';})
   .replace(/^(\d+) Nachzuchten in den normalen Bestand übernehmen\? Alle Daten und Historien bleiben erhalten\.$/,function(_,count){return 'Move '+count+' offspring animals to the regular collection? All data and history will be retained.';})
   .replace(/^(\d+) Nachzucht(?:en)? (?:wurde|wurden) in den Bestand übernommen\.$/,function(_,count){return count+' offspring animal'+(count==='1'?' was':'s were')+' moved to the collection.';})
 ...
 if(translated===trimmed&&currentLanguage==='it'){
  translated=translated
   ...
   .replace(/^(\d+) ausgewählt$/,function(_,count){return count+' selezionati';})
   .replace(/^(\d+) Nachzuchten in den normalen Bestand übernehmen\? Alle Daten und Historien bleiben erhalten\.$/,function(_,count){return 'Spostare '+count+' esemplari nel patrimonio normale? Tutti i dati e la cronologia verranno conservati.';})
   .replace(/^(\d+) Nachzucht(?:en)? (?:wurde|wurden) in den Bestand übernommen\.$/,function(_,count){return count+' esemplari spostati nel patrimonio.';})
 ...
 if(translated===trimmed&&currentLanguage==='hu'){
  translated=translated
   ...
   .replace(/^(\d+) ausgewählt$/,function(_,count){return count+' kiválasztva';})
   .replace(/^(\d+) Nachzuchten in den normalen Bestand übernehmen\? Alle Daten und Historien bleiben erhalten\.$/,function(_,count){return count+' utód áthelyezése a normál állományba? Minden adat és előzmény megmarad.';})
   .replace(/^(\d+) Nachzucht(?:en)? (?:wurde|wurden) in den Bestand übernommen\.$/,function(_,count){return count+' utód áthelyezve az állományba.';})
