...
function loadScriptOnce(moduleName,path,routeName,args,options){
 if(NGT500.modules[moduleName]){
  NGT500.route(routeName,args||{},options||{});
  return;
 }
 const existing=document.querySelector('script[data-tc-module="'+moduleName+'"]');
 if(existing){
  existing.addEventListener('load',function(){NGT500.route(routeName,args||{},options||{});},{once:true});
  return;
 }
 ...
 s.onerror=function(){
  console.error('Modul konnte nicht geladen werden:',moduleName,path);
  if(NGT500.toast){NGT500.toast('Ansicht konnte nicht geladen werden.','danger');}
 };
 document.head.appendChild(s);
}

function loadAnalytics(args,options){loadScriptOnce('analytics','./v500/modules/analytics.js','analytics',args,options);}
function loadSettings(args,options){loadScriptOnce('settings','./v500/modules/settings.js','settings',args,options);}
function loadReleaseTest(args,options){loadScriptOnce('releaseTest','./v500/modules/release-test.js','releaseTest',args,options);}
function loadAccount(args,options){loadScriptOnce('account','./v500/modules/account.js','account',args,options);}
...
function openMissingRoute(event){
 ...
 switch(event.name){
  ...
  case 'releaseTest':loadReleaseTest(event.args,options);break;
  case 'account':loadAccount(event.args,options);break;
  default:NGT500.route('dashboard',{},options);
 }
}

function navButton(icon,label,action,group){
 return '<button class="tc2DrawerItem '+(group||'')+'" onclick="'+action+'"><span>'+icon+'</span><b>'+label+'</b></button>';
}
...
function init(){
 ...
 if(menuButton)menuButton.onclick=NGT500.openMenu;
 if(overlay)overlay.onclick=NGT500.closeMenu;
 nav();
 NGT500.on('route-missing',openMissingRoute);
 const restored=NGT500.restoreRoute();
 if(!restored){NGT500.route('dashboard',{}, {replace:true,noHistory:true});}
}
