...
function normalizeFoodItem(f){f=f||{};f.name=f.name||f.label||'';f.key=f.key||foodKey(f.name);f.id=f.id||('food_'+f.key);f.label=f.label||foodLabel(f.name);f.qty=Number(f.qty||0);return f}
function normalize(d){d=d||base();TYPES.forEach(t=>{if(!Array.isArray(d[t]))d[t]=[];d[t].forEach((a,i)=>{a.uuid=a.uuid||a.uid||NGT500.uid();a.uid=a.uid||a.uuid;a.type=a.type||t;a.name=a.name||a.displayId||`${t}-${i+1}`;a.status=a.status||'Bestand';a.feeds=Array.isArray(a.feeds)?a.feeds:[];a.sheds=Array.isArray(a.sheds)?a.sheds:[];a.weights=Array.isArray(a.weights)?a.weights:[];a.photos=Array.isArray(a.photos)?a.photos:[];a.feeds.sort(byDate);a.sheds.sort(byDate);a.weights.sort(byDate);a.defaultFeeder=a.defaultFeeder||a.futterStandard||a.standardFeed||'';a.defaultFeederKey=a.defaultFeederKey||foodKey(a.defaultFeeder);});});['clutches','sales','archive','foodInventory'].forEach(k=>{if(!Array.isArray(d[k]))d[k]=[]});d.foodInventory=d.foodInventory.map(normalizeFoodItem);return d}
...
function addAnimal(t,a){a=a||{};a.uuid=a.uuid||NGT500.uid();a.uid=a.uid||a.uuid;a.type=t;a.feeds=a.feeds||[];a.sheds=a.sheds||[];a.weights=a.weights||[];a.photos=a.photos||[];a.defaultFeederKey=a.defaultFeederKey||foodKey(a.defaultFeeder||a.futterStandard||a.standardFeed||'');db[t].push(a);save();return a}
function updateAnimal(t,i,a){a.defaultFeederKey=a.defaultFeederKey||foodKey(a.defaultFeeder||a.futterStandard||a.standardFeed||'');db[t][Number(i)]=a;save();return a}
function deleteAnimal(t,i){db[t].splice(Number(i),1);save()}
function addFood(name,qty){const key=foodKey(name);const item=db.foodInventory.find(x=>x.key===key||foodKey(x.name)===key);if(item){item.name=name;item.label=foodLabel(name);item.key=key;item.id=item.id||('food_'+key);item.qty=Number(qty||0)}else db.foodInventory.push(normalizeFoodItem({name,qty:Number(qty||0)}));save()}
function reduceFood(name,n){const key=foodKey(name);const item=db.foodInventory.find(x=>x.key===key||foodKey(x.name)===key);if(item)item.qty=Math.max(0,Number(item.qty||0)-Number(n||1));save()}
