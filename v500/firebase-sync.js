...
let app=null,auth=null,db=null,user=null,mods=null;
let started=false,loading=false,saving=false,timer=null;

function read(k){try{return JSON.parse(localStorage.getItem(k)||"{}")||{}}catch(e){return {}}}
function write(k,v){localStorage.setItem(k,JSON.stringify(v||{}))}
function state(v){if(v)write(STATE_KEY,Object.assign(read(STATE_KEY),v));return read(STATE_KEY)}
function refresh(){try{if(window.NGTDashboard&&NGTDashboard.updateCloudStatus)NGTDashboard.updateCloudStatus()}catch(e){}}
function setStatus(status,msg){state({status,message:msg||"",at:new Date().toISOString()});refresh()}
function animalCount(){try{return NGTStore.allAnimals().length}catch(e){return 0}}
...
async function loadCloud(){
  ...
  try{
    ...
    if(snap.exists()){
      ...
      if(cloud.data){
        const cloudCount=["koenig","boas","geckos","spinnen"]
          .reduce((n,k)=>n+((cloud.data[k]||[]).length),0);

        if(cloudCount>0 || animalCount()===0){
...
async function saveCloud(){
  if(!user||loading||saving)return false;
  saving=true;
...
async function signIn(){
  ...
  saveProfile(user);
  await loadCloud();
  if(animalCount()>0)await saveCloud();
...
window.NGTFirebaseSync={start,signIn,signOut,loadCloud,saveCloud,label,state};
